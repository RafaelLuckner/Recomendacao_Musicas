...
def get_cover_url(song, artist, sp, file_path="data/cover_urls.csv"):
    # Load local cover_urls.csv
    if os.path.exists(file_path):
        cover_df = pd.read_csv(file_path)
        song = song.strip().lower()
        artist = artist.strip().lower()
        mask = (cover_df['track_name'].str.lower() == song) & (cover_df['artists'].str.lower() == artist)
        if mask.any():
            return cover_df.loc[mask, 'cover_url'].iloc[0], artist
    ...
    if cover_url and resolved_artist:
        ...
        cover_df = pd.concat([cover_df, new_entry], ignore_index=True)
        cover_df.to_csv(file_path, index=False)
        return cover_url, resolved_artist
            ...
            for _, row in sampled_rows.iterrows():
                ...
                if song not in recommendations:
                    ...
                    if pd.isna(cover_url):  
                        cover_url, resolved_artist = get_cover_url(song, artist, sp)
                    else:
...
def generate_recomendations_by_user_simmilarity(selected_genres, data, sp, limit=30):
    ...
    recommendations = {}
    for song, artist, genre, cover_url in zip(
        musics_recommendations["track_name"], 
        musics_recommendations["artists"],
        musics_recommendations["track_genre"], 
        musics_recommendations["cover_url"]
    ):
...
def show():
    ...
    # Check if cover_urls.csv needs updating (once per day)
    ...
    if last_update != today:
        update_cover_urls.update_cover_urls_file()
    ...
    with tab3:
        ...
        with col1:
            ...
            if st.session_state.get("selected_genres"):
                ...
                cols = st.columns(2)
                for i, genre in enumerate(["pop", "rock", "electronic", "hiphop", "jazz", 'sertanejo']):
                    with cols[i % 2]:
